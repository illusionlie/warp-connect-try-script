...
for %%i in (v4 v6) do (
	if exist ".\ips-%%i.txt" (
		2>nul >nul findstr /B /c:"#WARP-Connect-Script-IPsFile" ".\ips-%%i.txt"||(call :logger WARN BootCheck "IPs-%%iÊý¾ÝÒÑ¹ýÆÚ-½«ÖØÐÂÏÂÔØ" & del /f /q "ips-%%i.txt" >nul 2>nul)
		set "_ipsver-c="
		for /f "tokens=1,2 delims==" %%m in (.\ips-%%i.txt) do (
    		if "%%m"=="#Update" (set "_ipsver-c=%%n")
		)
		if /i !_ipsver! GTR !_ipsver-c! (
			call :logger INFO BootCheck "IPs-%%iÊý¾ÝÒÑ¹ýÆÚ-½«ÖØÐÂÏÂÔØ"
...
echo.#´ËÎª½Å±¾ÅäÖÃÎÄ¼þ°æ±¾ºÅ, ÇëÎðÐÞ¸Ä
echo._ipsver=20241003
echo.#IPsÈÕÆÚ, ÇëÎðÐÞ¸Ä
echo._loop=1
echo.#Ñ­»·Ò»´ÎµÄÊ±¼ä, µ¥Î»ÎªÃë
echo._check=10
echo.#´¥·¢Á¬½ÓÊ§°ÜµÄ´ÎÊý
echo._ipver=v6
echo.#ÒªÊ¹ÓÃµÄIP °æ±¾, v4 »ò v6
echo._log=true
echo.#ÊÇ·ñÆô¶¯ÈÕÖ¾¼ÇÂ¼
echo._warpmode=warp
echo.#ÒªÊ¹ÓÃµÄWARP Ä£Ê½, Ä¬ÈÏÎªwarp, Ê¹ÓÃ`warp-cli mode --help`²é¿´¿ÉÓÃµÄÖµ
echo._protocol=MASQUE
echo.#ÒªÊ¹ÓÃµÄWARP Ð­Òé, Ä¬ÈÏÎªMASQUE, Ê¹ÓÃ`warp-cli tunnel protocol set --help`²é¿´¿ÉÓÃµÄÖµ
echo._renewnum=3
echo.#´¥·¢ÖØÐÂ»ñÈ¡¶ËµãµÄ´ÎÊý
echo._proxydetect=true
echo.#ÊÇ·ñ¼ì²â´úÀíÒÑ¿ªÆô^(±ÜÃâÁ¬½ÓÎÊÌâ^)
echo._nosleep=false
echo.#ÊÇ·ñÔÚ½Å±¾ÔËÐÐÊ±×èÖ¹ÏµÍ³Ë¯Ãß
echo._notice=true
echo.#ÊÇ·ñÔÚ½Å±¾ÍË³öÊ±ÏÔÊ¾ÆøÇòÍ¨Öª
echo._updater=true
echo.#ÊÇ·ñÆô¶¯×Ô¶¯¼ì²é¸üÐÂ
)
goto :eof
